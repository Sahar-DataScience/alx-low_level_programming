...
dog_t *new_dog(char *name, float age, char *owner)
{
	...
	boby_dog = malloc(sizeof(dog_t));
	if (boby_dog)
	{
		boby_dog->name = malloc(sizeof(char) * (name_len + 1));
		if (boby_dog->name)
		{
			boby_dog->owner = malloc(sizeof(char) * (owner_len + 1));
			if (boby_dog->owner)
			{
				my_strcpy(boby_dog->name, name);
				boby_dog->age = age;
				my_strcpy(boby_dog->owner, owner);
				return (boby_dog);
			}
			else
			{
